...
function setupComments(commentsData) {
  return new Promise(function(resolve, reject) {
    commentFactory
      .createMany('comment', commentsData)
      .then(commentsArray => {
...
describe('GET /Comment', () => {
  ...
  test('can search based on commentPeriodId', done => {
    commentPeriodFactory.create('commentperiod', { name: 'Open Season comment period' }).then(commentPeriod => {
      commentFactory
        .create('comment', { name: 'Rah Rah very angry!', _commentPeriod: commentPeriod.id })
        .then(comment => {
          request(app)
            .get('/api/comment')
