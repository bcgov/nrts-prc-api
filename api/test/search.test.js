...
describe('GET /api/public/search/bcgw/getClientsInfoByDispositionId', () => {
  ...
  describe('When the arcgis call returns successfully', () => {
    beforeEach(() => {
      argGis.get(searchPath + urlEncodedDispositionId).reply(200, arcGisResponse);
    });
  ...
  describe('When the arcgis call returns with an error', () => {
    ...
    beforeEach(() => {
      argGis.get(searchPath + urlEncodedDispositionId).reply(400, arcGisErrorResponse);
    });
  ...
  describe('when the arcgis call returns 200, but error in body', () => {
    ...
    beforeEach(() => {
      argGis.get(searchPath + urlEncodedDispositionId).reply(200, arcGisErrorResponse);
    });
...
describe('GET /api/public/search/bcgw/crownLandsId/ ', () => {
  ...
  const searchPath =
    '/geo/pub/WHSE_TANTALIS.TA_CROWN_TENURES_SVW/ows?service=wfs&version=2.0.0&request=getfeature&typename=PUB:WHSE_TANTALIS.TA_CROWN_TENURES_SVW&outputFormat=json&srsName=EPSG:4326&CQL_FILTER=CROWN_LANDS_FILE=';
  ...
  describe('when bcgw call is successful', () => {
    beforeEach(() => {
      bcgw.get(searchPath + paddedCrownlandsId).reply(200, crownlandsResponse);
    });

    test('returns the features data from the search', done => {
      request(app)
        .get('/api/public/search/bcgw/crownLandsId/' + crownlandsId)
        .expect(200)
        .then(response => {
  ...
  describe('When the bcgw call returns with an error', () => {
    ...
    beforeEach(() => {
      bcgw.get(searchPath + paddedCrownlandsId).reply(400, bcgwErrorResponse);
    });
  ...
  describe('when the bcgw call returns 200, but error in body', () => {
    ...
    beforeEach(() => {
      bcgw.get(searchPath + paddedCrownlandsId).reply(200, bcgwErrorResponse);
    });
...
describe('GET /api/public/search/bcgw/dispositionTransactionId', () => {
  const bcgwDomain = 'https://openmaps.gov.bc.ca';
  const searchPath =
    '/geo/pub/WHSE_TANTALIS.TA_CROWN_TENURES_SVW/ows?service=wfs&version=2.0.0&request=getfeature&typename=PUB:WHSE_TANTALIS.TA_CROWN_TENURES_SVW&outputFormat=json&srsName=EPSG:4326&CQL_FILTER=DISPOSITION_TRANSACTION_SID=';
  const bcgw = nock(bcgwDomain);
  let dispositionId = 666666;
  let urlEncodedDispositionId = `%27${dispositionId}%27`;
  ...
  describe('When the bcgw call returns successfully', () => {
    beforeEach(() => {
      bcgw.get(searchPath + urlEncodedDispositionId).reply(200, tantalisResponse);
    });
  ...
  describe('When the bcgw call returns with an error', () => {
    ...
    beforeEach(() => {
      return bcgw.get(searchPath + urlEncodedDispositionId).reply(400, bcgwErrorResponse);
    });
  ...
  describe('when the bcgw call returns 200, but error in body', () => {
    ...
    beforeEach(() => {
      return bcgw.get(searchPath + urlEncodedDispositionId).reply(200, bcgwErrorResponse);
    });
